...
em hex estão guardados (tabela de operação de códigos disponível heim disponível em http://www.emulator101.com/reference/8080-by-opcode.html) e retornará os bytes de operação*/
#include <stdio.h>
int dissassemblerIntel8080(unsigned char *codePtr, int pc) {
    unsigned char *code = &codePtr[pc]; //o ponteiro para char code recebe o endereço da última operação
    int byte = 1; //cada operação tem, no mínimo, 1 byte, então escrevendo aqui salva tempo. Serve para indicar o quanto o ponteiro irá pular na leitura após a execução da função
    printf("instrução #%04x    ", pc);
    switch(*code) {
        case 0x00:
            printf("NOP");
            break;
        ...
        case 0x88:
            printf("ADC    B");
            break; //ô caralho essa porra não acaba
        case 0x89:
            printf("ADC    C");
            break;
        case 0x8a:
